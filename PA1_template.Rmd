...
daily_totals <- sapply(daily_steps, function(s) sum(s[, 'steps'], na.rm=TRUE))
...
interval_totals <- sapply(interval_steps, function(s) sum(s[, 'steps'],
                                                          na.rm=TRUE))
...
max_loc <- steps_per_interval == max_value
max_interval <- as.numeric(names(steps_per_interval[max_loc]))
```
...
```{r}
num_missing <- sum(!complete.cases(raw_data$steps))
```
